    ...
    function (err, res, body) {
      ...
      if (!err && res && res.statusCode && res.statusCode === 200) {
        ...
        async.detect(
          ...
          function (matchingItem, cb) {
                  if (!matchingItem || !matchingItem.endpoints) {
              return callback(new Error('storageName not found: ' + storageName), res, tokens);
            }
            var endpoints = matchingItem.endpoints;
            if (!Array.isArray(endpoints) || !endpoints[0]) {
              return callback(new Error('Empty endpoints array'), res, tokens);
            }
            var endpoint = endpoints[0];
            if (options.region) {
              for (var i = 0; i < endpoints.length; i++) {
                if (endpoints[i].region === options.region) {
            ...
            if (endpoint.hasOwnProperty(storageURLAttribute)) {
              tokens.storageUrl = endpoint[storageURLAttribute];
            } else {
